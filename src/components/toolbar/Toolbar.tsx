...
const Toolbar = () => {
    return (
        <div
            className="box-border bg-background-toolbar drop-shadow-ds-001 flex gap-4 justify-center items-center 
        text-white p-1 h-14 lg:h-10 w-full"
        >
            ...
            <div className="box-border flex gap-2 justify-center items-center md:px-10 h-full w-full md:w-fit">
                {tools.map(({ id, name, icon }) => (
                    <ToolItem key={id} id={id} name={name} icon={icon} onClick={() => {}} />
                ))}
            </div>
